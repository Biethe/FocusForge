...
**STATUS: Phase 6 G1-G4 done, device sync live, one wrong theory reverted (2026-08-02). :governor ("aarchmage") pure Kotlin/JVM: CpuTopology, PerformanceContract, CostModel, SelfBenchmark, ProfileDeriver, Governor — all tested, 183+ tests across modules, CI fails if :governor is absent from the arm64 build. adb-over-WiFi sync works (scripts/sync-device.sh); the operator no longer exports by hand. INVESTIGATION CLOSED (docs/SIGNALS.md §17): one drowsy recording measures PERCLOS 0.000. The operator's theory (recorded before the PERCLOS fix) is wrong — replay recomputes from raw landmarks with today's code. MY theory (eye geometry too narrow, P80 arithmetically unreachable below earOpen 0.15) was also wrong, was implemented as a PerclosValidity flag, flagged the WRONG recording when measured, and has been REVERTED. Root cause of my error: bench/analyze_blinks.py still replicated the frozen 5 s calibration after the engine moved to a rolling one, reporting earOpen 0.101 where the engine computes 0.274 — the script is now fixed. Measured with the engine, that session's earOpen is normal and its deepest closure is 0.744: the eyes genuinely never closed past P80, and long closures caught it. The original finding stands and is the best evidence for the multi-signal design (§15.8). Remaining: G5 CI cross-silicon exhibit, G6 closeout, wiring the real backend on-device, airplane-mode screenshot.** (Update this line at the end of every session.)
