...
**STATUS: The coach generates text on the A20e — Phase 5 is functionally complete (2026-08-02). Measured alongside a live camera session: TTFT 9727 ms, 7.6 tok/s, RSS peak 580 MB. Decode and memory PASS; TTFT FAILS the 3000 ms contract by 3x, with both causes identified: (a) prompt length dominates — TTFT includes processing every prompt token, and the coach prompt was ~130 tokens against the smoke test's 22, which at 1240 ms scales to ~7300 ms; (b) CPU contention — decode also fell 13.0 -> 7.6 tok/s with the camera live, where prompt length plays no part. Fixed: prompt halved (479 -> 232 chars, a test now fails above 400 chars with the measurement in the message), and the vision loop stands down while the model works (FacePipeline.paused) — a preview of the Phase 6 governor's fps-budget knob, and the first evidence the knob is worth having. Also fixed the operator-reported bug that a long coach message pushed the export button off-screen (the layout was not scrollable). NO PROJECTED TTFT IS RECORDED — the next on-device run is the only honest number. 120 :core tests pass. Awaiting operator: run 0.5.5-latency in AIRPLANE MODE, screenshot a coaching message with the airplane icon (required submission artifact), export and send the session JSON. Then the :governor pivot — this TTFT violation is its case study.** (Update this line at the end of every session.)
