...
**STATUS: Phase 6 G1-G4 built; device sync live (2026-08-02). :governor ("aarchmage") is pure Kotlin/JVM, runs on the arm64 CI runner, and CI fails if it is absent there. Implemented and tested: CpuTopology discovery (same code describes the A20e's 2+6 clusters and the runner's uniform 4), PerformanceContract (battery limit deliberately null — unmeasured, so recorded not enforced), CostModel (ttft = fresh tokens x ms/token, fitted on the operator's 9 real runs, reproduces them within 5%, picks CHEAPEST compliant not fastest, returns null when nothing complies), SelfBenchmark (three probes, respects a 60 s budget using its own measurements, reports complete=false when truncated), ProfileDeriver (every knob carries the measurement behind it; a preference is never dressed as a result), and Governor (one knob per decision, hysteresis both edges, only the fps budget actuated this cycle with n_ctx and threads derived-and-logged, every decision carries its trigger). ADB-over-WiFi sync is working (scripts/sync-device.sh) — the operator never exports by hand again. That immediately surfaced 6 unseen recordings: there are now 3 focused / 2 distracted / 3 drowsy, and AllRecordingsOrderingTest asserts the worst of one label beats the best of another. NEW NEGATIVE RESULT (docs/SIGNALS.md §17): PERCLOS alone does NOT separate every drowsy session — one measures 0.000, identical to focused, and is caught only by long closures. Strongest evidence yet for the multi-signal design, and only visible because n>1. Score reproducibility across three focused sessions: 96.7/96.7/96.6. Remaining: G5 CI cross-silicon exhibit, wiring the real backend to the harness on-device, and the airplane-mode screenshot.** (Update this line at the end of every session.)
