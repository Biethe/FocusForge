...
**STATUS: TTFT CONTRACT MET (2026-08-02). Operator benchmark (bench/results/a20e-threads-kvcache-20260802.json): prefill runs at 16.9 / 24.0 / 31.2 tok/s on 2 / 4 / 6 threads, so 6 threads takes a cold coaching message from 4913 to 2659 ms and a warm one to 1481 ms against a 3000 ms contract — passing with 2x headroom, and passing even cold. KV reuse cuts TTFT 38-44% at every thread count. App default changed 2 -> 6 threads. NOTE FOR THE ARCHITECT: this contradicts CLAUDE.md §5's posture ("LLM generation on 2 threads toward the A73 cluster"), which was a documented starting point, never measured; §5's text is deliberately NOT edited — that needs architect sign-off. Scaling is sub-linear (3x threads, 1.85x speed) and thread AFFINITY is still untested, so §5 may be right about placement while wrong about count. Derived cost model: ttft = (prompt - reused) x ms_per_fresh_token, fitted on cold runs only and predicting unseen warm runs to within 1.7% — this is what lets the Phase 6 governor PREDICT contract compliance rather than explore for it. Coach message quality confirmed good on-device. Awaiting operator: one session on 0.5.8-threads6 in AIRPLANE MODE and the screenshot with the airplane icon — the last artifact Phase 5 needs. Then the :governor pivot.** (Update this line at the end of every session.)
