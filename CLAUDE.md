...
**STATUS: Phase 3 fully closed (2026-07-31). The operator's three 2-minute recordings are committed (100% face visibility) and all 66 :core tests pass, including the three replay ordering assertions that were skipped until now — PERCLOS drowsy 0.118 vs focused 0.000, gaze focused 0.984 vs distracted 0.599, long closures 16 vs 0. PERCLOS was structurally broken (0.80 applied to a confidence score that peaks at 0.73); fixed by measuring eye closure geometrically against the user's own calibrated open eye — the threshold itself was not touched (docs/DECISIONS.md, docs/SIGNALS.md §5.1). Known and documented: our PERCLOS reads low by design, and yawn detection does not work on this device (jawOpen peaks at 0.612 vs a 0.60/1.2 s rule) — the fix needs mouth landmarks and so needs architect sign-off on the privacy allow-list. Outstanding: Phase 2 HUD numbers. Next: Phase 4 — focus score + session dashboard; note gaze alone cannot separate drowsy from distracted.** (Update this line at the end of every session.)
