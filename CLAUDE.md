...
**STATUS: PHASE 6 COMPLETE (2026-08-02). :governor ("aarchmage") built, tested, and wired into the app. Pure Kotlin/JVM with zero Android imports — the reason the arm64 runner can execute the same self-benchmark path; CI fails if :governor is absent there. CROSS-SILICON EXHIBIT committed at bench/profiles/: the A20e (8 cores, 2 clusters, no dotprod/i8mm/SVE) derives 4 threads / 2190 ms predicted, the ubuntu-24.04-arm runner (4 cores, 1 cluster, all those features) derives 1 thread / 144 ms — same code path, neither configured. The runner uses a reference CPU kernel not the model, recorded in its own device.workload_note; thread scaling and sustained-load behaviour are comparable, milliseconds are not. IN THE APP: LLM screen button 4 runs the real self-benchmark and writes device.profile.json; SessionActivity loads it, sets the coach's thread count and the vision fps budget, and runs the governor every 30 s against the contract, dropping frames before MediaPipe rather than reconfiguring the camera. Every decision and the profile go into the session export. Only the fps budget is actuated (n_ctx and threads derived+logged with applied=false and the reason), as the approved plan agreed. Two bugs the first real run exposed and the simulation could not: a reference unit smaller than the millisecond clock made least squares fit a NEGATIVE cost per unit (unit now 8.4M MACs), and CostModel.fit now rejects a non-positive slope. AWAITING OPERATOR: install 0.6.0-governor, run LLM screen -> button 4 (~90 s), then a normal session; I pull both files over adb. STILL OUTSTANDING: the airplane-mode screenshot (last Phase 5 artifact), and Phase 7 submission materials. The governor's reacting path has never seen a real contract violation on the phone because the coach now meets its targets — tested in simulation only.** (Update this line at the end of every session.)
