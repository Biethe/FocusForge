...
**STATUS: Architect's blink/fps ruling applied in full (2026-08-02), recorded verbatim in docs/DECISIONS.md. Blink rate is display/telemetry only — WEIGHT_BLINK_RATE = 0.0, with a test proving the score is identical across a 10x blink-rate change; the fusion and fatigue flag rest on PERCLOS, long closures, gaze and head stability, all fps-robust. Exports now carry visionFps per sample window plus blinkRateValidity ("full-rate" | "undersampled") on every row and on the totals; both screens show `blinks N+ (rate undersampled at X fps)` rather than a rate. Full-rate line is 15 fps, derived from the measured p50 blink duration of 132 ms — the A20e measures 8.4-9.6 fps so this reads `undersampled` permanently and by design. Phase 4.5 assertions unchanged and passing; 98 :core tests green. Event-driven burst sampling recorded as DEFERRED (not before Phase 6B, and it would sharpen closure timing without recovering missed blinks). Outstanding: Phase 2 HUD numbers. Next: Phase 5 — on-device LLM coach (SmolLM2-360M Q4 via llama.cpp, armv8-a baseline only, verify it runs on the A20e before calling it done).** (Update this line at the end of every session.)
