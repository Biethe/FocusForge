...
**STATUS: First coach session run; memory confirmed, two bugs found and fixed (2026-08-02). RSS peaked at 560 MB with camera + model both live — under the 700 MB budget and below the 598 MB projection, so coexistence is settled. Session committed at bench/sessions/session-sm-a202f-20260802-155732.json. BUG 1: the coach never spoke despite the fatigue flag being up for 56% of the session — CoachPolicy tested a rising edge but consumed it during warm-up, so an episode starting at t=46 s (warm-up ends at 60 s) could never fire; replaced with episode tracking. BUG 2: earOpen calibrated to 0.188 against a true ~0.28 because the user looks DOWN at the phone to tap Start and the reference was a median FROZEN at 5 s; now a rolling median. A p90 variant was tried first and REVERTED — it made every frame read more closed, amplifying the look-down confound and false-firing fatigue on the distracted recording (long closures 2 -> 6). 119 :core tests pass, 5 new regression tests from these failures; all replay orderings still hold. NOT MEASURED YET: the coach generating a single word on the phone. Awaiting operator: run 0.5.4-coachfix in AIRPLANE MODE, screenshot a coaching message with the airplane icon (required submission artifact), send the session JSON; if still silent, report the grey coach status line. Then the :governor pivot.** (Update this line at the end of every session.)
