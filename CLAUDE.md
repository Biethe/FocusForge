...
**STATUS: Phase 4.5 patch complete (2026-08-01) — blink detection retuned from measured data. Blinks peaked at a median depth of 0.51 against a 0.50 threshold, so half were missed; blinks now use 0.30/0.18 while long closures keep 0.50/0.35 as a separate state machine, because the eye aspect ratio also falls when looking down and a shared low threshold turned a distracted session into 60% false fatigue alarms. Focused blink rate 9.4 -> 16.4/min, long closures still 0/2/14, score and fatigue flag unchanged (96.7/74.9/60.7). 89 :core tests pass including a new tripwire that a focused session must show 3-30 blinks/min. Full reasoning and measurements: docs/SIGNALS.md §16. Awaiting operator: install 0.4.1-blinks, report the `eyes ref` open-EAR value, and record the 30 s `blinkprobe` (§16.6) — plus the session JSON from 18:50 on 2026-07-31 if it still exists. STILL UNEXPLAINED (§16.5): that session's max eyeClosure of 0.219; leading hypothesis is a low open-eye calibration, now exposed as `earOpen` in the panel and the export. Also outstanding: Phase 2 HUD numbers. Next: Phase 5 — on-device LLM coach.** (Update this line at the end of every session.)
