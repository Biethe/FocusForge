...
**STATUS: SIGILL RISK RETIRED ON HARDWARE (2026-08-02). The operator ran 0.5.0 on the A20e and it did NOT crash — native library, model load, context init and llama_decode all executed on Armv8.0-A. The Armv8.0 baseline work is proven on the device, not just statically. The run failed with `no tokens generated`, root-caused to feeding a bare prompt to a ChatML instruct model: its first predicted token was end-of-generation, so the loop broke before counting. Fixed by applying the model's own chat template (needed for the coach anyway), plus much richer diagnostics — prompt token count, first token id, whether it was EOG, decode return code, template applied — because `no tokens generated` cost an on-device round trip and told us nothing. Awaiting operator: install 0.5.1-chat-template and re-run; TTFT / tok-s / RSS are still NOT MEASURED YET. Note the numbers will come from q8_0 (386 MB) since the official repo publishes no Q4_K_M; they are a floor, not the Phase 5 result. Gate deadline end of Aug 4 — inside it, with the dangerous part done. Then Phase 5.1/5.2 coach UI, then the :governor pivot.** (Update this line at the end of every session.)
