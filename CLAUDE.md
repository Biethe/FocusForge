...
**STATUS: PHASE 5 GATE PASSED (2026-08-02, two days early). SmolLM2 q8_0 generated 20 tokens on the A20e at 11.4 tok/s decode against a floor of 5 — llama.cpp runs on Armv8.0-A. CAVEAT ON THE RECORD: the reported "TTFT 3149 ms" is NOT TTFT; the first implementation started the clock before llama_model_load_from_file, so it is cold start including mapping a 386 MB file off eMMC. Rewritten to hold the model open behind a handle (nativeLoad/nativeGenerate/nativeFree) with load time and TTFT measured separately — needed anyway for the coach and for the governor's 60 s self-benchmark. Smoke screen now runs cold + warm on one open model. STILL MISSING: on-device RSS, which amendment 2 makes a stop condition (>700 MB = halt and report). Awaiting operator: install 0.5.2-warm-ttft, re-run, send the MEMORY block. All numbers so far are q8_0 (386 MB) and are a floor; the shipping quant is Q4_K_M (271 MB). Then Phase 5.1/5.2 coach UI, then the :governor pivot.** (Update this line at the end of every session.)
