...
**STATUS: Phase 5 gate passed and the coach is built (2026-08-02). Memory coexistence RESOLVED: operator measured a running session at 177 MB, so the camera+MediaPipe pipeline adds only 44 MB over the base app; session+q8_0 = 598 MB and session+Q4_K_M = 483 MB against the 700 MB budget. The model therefore stays resident for the whole session (1.2 s per message, not 3.1 s). Coach implemented: trigger policy and prompt construction live in :core and are unit-tested (114 tests, 16 new, mostly asserting SILENCE — no warm-up messages, no reaction to brief dips, never two inside 5 minutes, no stale milestones); CoachRunner in :app owns the open model and one worker thread, dropping rather than queueing a trigger while a generation is in flight. EN/FR toggle; every message shows its measured TTFT and tok/s and both go into the session export with the trigger that caused it. NOT MEASURED YET: the coach on the phone. Awaiting operator: install 0.5.3-coach, run a session in AIRPLANE MODE, screenshot a coaching message with the airplane icon (required submission artifact), and export the session JSON. Then the :governor pivot (plan approved).** (Update this line at the end of every session.)
