...
**STATUS: PHASE 5 GATE PASSED ON ALL THREE TARGETS (2026-08-02, two days early). Measured on the A20e, build 0.5.2, q8_0/386 MB, 2 threads, n_ctx 512: TTFT 1240 ms (target <=3000), decode 13.0 tok/s (target >=5), RSS peak 557 MB (budget 700). Two runs agree to 1% on decode. Full record: bench/results/a20e-phase5-gate-20260802.json. All figures are FLOORS — the shipping quant is Q4_K_M at 271 MB vs the 386 MB q8_0 tested. NEW RISK, now on the critical path: that 557 MB was measured with the camera NOT running. A real session holds CameraX + MediaPipe + the model simultaneously and the costs add; the LLM alone takes 421 MB of the 700 MB budget (Q4_K_M ~306 MB). The Phase 2 camera-screen RSS has been outstanding since Phase 2 and now decides whether Q4_K_M is a preference or a requirement — requested from the operator. Next: Phase 5.2 coach UI (event-triggered, EN/FR, TTFT+tok/s shown and logged), then the :governor pivot.** (Update this line at the end of every session.)
